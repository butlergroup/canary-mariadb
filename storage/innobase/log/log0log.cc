                                             ...
                                             lsn_t offset) noexcept
{
  ...
  ut_ad(!is_mmap());
  ut_ad(is_latest());
  /* An assertion failure here means that we failed to complete a log
  checkpoint quick enough and would have to start writing to a new log file
  (or start to extend this log file). In other words,
  the innodb_log_file_size is too small for this workload.

  A failure can easily be reproduced with a write-heavy workload
  and the minimum innodb_log_file_size=4m.

  With innodb_buffer_pool_size=16G and innodb_log_file_size=512M
  (1/32 of the buffer pool size) there is no crash, but the system
  could be stuck in checkpoint flush for 3/4 of the time.

  Good write performance requires reasonably large innodb_log_file_size. */
...
ATTRIBUTE_COLD void log_t::checkpoint_margin() noexcept
{
  ...
  while (check_for_checkpoint())
  {
    ...
    const lsn_t max_age{max_checkpoint_age + archive * file_size};
    lsn_t lsn{first_lsn};
